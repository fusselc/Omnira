...
Three pinned llama.cpp `llama-server` Windows builds ship in the installer:

- **CUDA 12.4 x64** -- preferred on NVIDIA GPUs (cudart redistributable DLLs
...
require no architectural change. An optional LocalAppData drop-in at
`%LOCALAPPDATA%\Omnira\runtimes\<variant>\` is also searched.
