...
#
# Downloads the pinned llama.cpp llama-server Windows builds (Vulkan, CPU, and
...
#
# CUDA also unpacks the matching cudart redistributable into binaries/cuda so
# NVIDIA users do not need a separate CUDA Toolkit install. Pass -SkipCuda to
# fetch only Vulkan + CPU and leave the CUDA resource slot empty.
#
...
# Checksums are the sha256 digests published on the ggml-org/llama.cpp release.
$PinnedTag    = "b9859"
...
foreach ($artifact in $Artifacts) {
    ...
    Expand-RuntimeVariant $zipPath $artifact.Variant
}

if ($SkipCuda) {
    $CudaPlaceholder = Join-Path $BinDir "cuda"
    New-Item -ItemType Directory -Force -Path $CudaPlaceholder | Out-Null
    Write-Host "[ready ] cuda: slot reserved at $CudaPlaceholder (empty; -SkipCuda)."
} else {
    ...
    $dlls = @(Get-ChildItem -Recurse -Path $cudartTmp -Filter "*.dll")
    if ($dlls.Count -eq 0) {
        Remove-Item -Recurse -Force $cudartTmp
        Write-Error "No DLLs found in $($CudartArtifact.Name); refusing to continue."
        exit 1
    }
    foreach ($dll in $dlls) {
        Copy-Item $dll.FullName -Destination $cudaDir -Force
    }
    Remove-Item -Recurse -Force $cudartTmp
