 ...
 * user can see what is actually running. Accelerator names never appear here
 * (not even in the tooltip); that detail lives in Advanced Diagnostics.
 */
export function StatusPill({ status }: { status: RuntimeStatus }) {
  ...
  const title = engine ? `Engine: ${engine}` : undefined;
